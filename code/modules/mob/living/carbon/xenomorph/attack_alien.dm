...
/mob/living/carbon/human/attack_alien(mob/living/carbon/xenomorph/attacking_xeno, dam_bonus)
	...
	switch(intent)
		...
		if(INTENT_GRAB)
			...
			//if(check_shields(0, attacking_xeno.name)) // Blocking check // RUCM REMOVE
			if(check_shields(0, attacking_xeno.name, attacker_dir = get_dir(attacking_xeno, src))) // RUCM ADD
